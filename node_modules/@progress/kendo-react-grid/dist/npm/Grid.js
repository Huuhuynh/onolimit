...
var Grid = /** @class */ (function (_super) {
    ...
    function Grid(props) {
        ...
        _this.columnResize = new ColumnResize_1.ColumnResize(_this.onResize.bind(_this));
        return _this;
    }
    ...
    Grid.prototype.componentWillReceiveProps = function (nextProps) {
        ...
        if (nextProps.total !== this.props.total ||
            ...
            this.vs.reset();
            this.vs = new VirtualScroll_1.VirtualScroll(groupable || nextProps.rowHeight === undefined || nextProps.rowHeight === 0);
        }
    ...
    Grid.prototype.render = function () {
        ...
        var data = [];
        var resolvedGroupsCount = index_1.flatData(data, propsData, groupingFooter, { index: this.props.skip || 0 }, this.props.group !== undefined, this.props.expandField);
        ...
        var dataRow = function (item) { return _this._columns.map(function (column, index) {
            ...
            if (column.editable && _this.props.editField) {
                ...
                if (inEdit === true || inEdit === column.field) {
                    return React.createElement(GridEditCell_1.GridEditCell, __assign({ key: index }, cellProps));
                }
            }
            return React.createElement(GridCell_1.GridCell, __assign({ key: index }, cellProps));
        }); };
        var hiddenRows = 0;
        ...
        var body = data.length && data.map(function (item, index) {
            ...
            return [(React.createElement(GridRow_1.GridRow, { key: absoluteIndex(index) * 2, dataItem: item.dataItem, isAltRow: isAlt, rowType: item.rowType, isHidden: hidden(index), onClick: function (e) { return _this.rowClick(e, item); }, onDoubleClick: function (e) { return _this.rowDblClick(e, item); }, selectedField: _this.props.selectedField, rowHeight: _this.props.rowHeight, render: _this.props.rowRender }, dataRow(item))),
                _this.props.detail && item.rowType === 'data' && item.expanded && (React.createElement("tr", { key: absoluteIndex(index) * 2 + 1, className: isAlt ? 'k-detail-row k-alt' : 'k-detail-row', style: { visibility: hidden(index) ? 'hidden' : '' } },
                    _this.props.group && _this.props.group.map(function (group, idx) {
                        return (React.createElement(GridGroupCell_1.GridGroupCell, { dataIndex: item.dataIndex, field: group.field, dataItem: item.dataItem, key: idx, style: {} }));
                    }),
            ...
            React.createElement("td", { colSpan: this._columns.filter(function (c) { return !c.children.length; }).length }, noRecords.length ? noRecords : React.createElement(GridNoRecords_1.GridNoRecords, null))));
        var pager = this.props.pageable && (React.createElement(Pager_1.Pager, { pageChange: this.pageChange, total: total, skip: this.vs.propsSkip || 0, pageSize: (this.props.take !== undefined ? this.props.take : this.props.pageSize) || 10, settings: this.props.pageable }));
            ...
            React.createElement("div", { className: "k-grid-container" },
                React.createElement("div", { ref: function (container) { _this.vs.container = container; }, className: "k-grid-content k-virtual-content", onScroll: this.scrollHandler },
                    React.createElement("div", { style: { 'position': 'relative' } },
                        React.createElement("table", { tabIndex: -1, className: 'k-grid-table', ref: function (table) { return _this.vs.table = table; } },
                            colGroups,
    ...
    Grid.prototype.initColumns = function (columnElements, groupCount) {
        var _this = this;
        this._columns = index_1.readColumns(columnElements, this.columns);
        if (this._columns.length === 0) {
            this._columns = index_1.autoGenerateColumns(this.props.data, this.props.group, this.props.expandField);
        }
        ...
        if (this.props.expandField && (this.props.onExpandChange) && this.props.detail) {
            this._columns.unshift(__assign({}, defaultServiceProps, { cell: GridHierarchyCell_1.GridHierarchyCell, field: this.props.expandField, headerClassName: 'k-hierarchy-cell k-header' }));
            columnIndexOffset++;
        }
        for (var i = 0; i < groupCount; i++) {
            this._columns.unshift(__assign({}, defaultServiceProps, { cell: GridGroupCell_1.GridGroupCell, field: 'value' }));
            columnIndexOffset++;
        }
